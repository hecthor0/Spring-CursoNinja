...
public class LimitsController {
	...
	public ResponseEntity<Limit> createLimit(@PathVariable("accountId") String accountId,
			@Valid @RequestBody Limit limit) {
		Limit returnLimit = limitsService.createLimit(limit);
		return ResponseEntity.status(HttpStatus.CREATED).body(returnLimit);
	}

	@ResponseStatus(HttpStatus.BAD_REQUEST)
	@ExceptionHandler(MethodArgumentNotValidException.class)
	public List<Message> handleValidationExceptions(MethodArgumentNotValidException ex) {
